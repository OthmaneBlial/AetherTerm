...
| Packaging and automation | A wheel installed in clean Python 3.13 environments served its bundled UI and completed an authorized PTY round trip on macOS and Ubuntu 24.04 CI. The non-root container, dependency scans, local-only Web assets, three browser journeys and Linux x86_64 agent binary with a real PTY passed in [run 35449348945](https://github.com/OthmaneBlial/AetherTerm/actions/runs/35449348945). No published release artifact is verified. | Publicly tested downloads, review gates and release checks. |
