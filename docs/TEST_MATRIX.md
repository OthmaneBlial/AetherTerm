...
This matrix maps the security and shell contract to executable checks. It describes coverage in this checkout, not a production audit. The [CI workflow](../.github/workflows/ci.yml) runs Python tests on Ubuntu 24.04 with Python 3.13, Chromium, a non-root container and a frozen Linux x86_64 agent. Local macOS checks add a second development environment.
...
| Browser can recover from a network loss without resuming an old shell | `scripts/verify_browser.py`: Chromium offline/online, real PTY before and after, plus server restart and reauthentication | Different networks, proxies and physical browsers |
...
The suite has intentional limits: no fixture contains a real credential, all test listeners stay on loopback, the fake agent in the output stress checks tests server relay behavior, and the Chromium test uses a real agent/PTY for user journeys. A green CI run does not prove external deployment, independent review or acceptance by target users. See [ROADMAP.md](../ROADMAP.md) for those gates.
