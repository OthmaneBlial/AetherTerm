...
2. Le navigateur se connecte avec le mot de passe. Le serveur émet un cookie HTTP seulement, lié à une session conservée en mémoire pendant au plus huit heures. `/web/` et `/ws` exigent cette session. Le POST de connexion/déconnexion et le WebSocket navigateur vérifient l'origine.
