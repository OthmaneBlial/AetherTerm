...
- [ ] 2.2 Terminal interactif et Unicode exact — xterm.js local, UTF-8 fragmenté, couleurs ANSI, `less` et redimensionnement testés avec Chrome et un PTY macOS ; collage Unicode réel et bouton « Send Ctrl+C » interrompant `cat`, puis commande suivante, vérifiés dans Chrome sur macOS ; Linux, `vim`, flèches et presse-papiers sur d'autres navigateurs restent à valider.
...
- [ ] 6.1 CI bloquante — run [35446357041](https://github.com/OthmaneBlial/AetherTerm/actions/runs/35446357041) vert sur `dbaf5e9` avec actions figées (Linux Python 3.13, PTY réel, wheel installé, assets Web, scans, Chromium et conteneur) ; captures navigateur conservées quand produites ; politique de blocage PR et contrôle de docs encore requis.
- [ ] 6.2 Artefacts téléchargeables/installables — wheel installé et testé sur macOS et Linux CI, serveur conteneur non-root testé avec un PTY sur Linux CI ; binaire agent Linux x86_64 construit, démarré, contrôlé par SHA-256 et utilisé pour un vrai PTY dans le run [35446843613](https://github.com/OthmaneBlial/AetherTerm/actions/runs/35446843613). L'artifact CI expire après sept jours ; installation depuis une release publiée, autres plateformes et second hôte non vérifiés.
