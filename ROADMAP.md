...
- [x] 2.3 Reconnexion et états d'erreur — ancien shell terminé et nouvel agent enregistré après redémarrage réel du serveur ; révocation arrête l'agent, backoff réinitialisé après succès. Le parcours Chromium Linux [35448700866](https://github.com/OthmaneBlial/AetherTerm/actions/runs/35448700866) a coupé le réseau du navigateur : état hors ligne détecté en 0,02 s, reconnexion en 0,45 s, puis nouveau shell réel ouvert ; il vérifie aussi réauthentification et nouveau shell après redémarrage du serveur. Délais propres à ce runner, sans garantie de réseau réel.
...
- [ ] 4.1 Architecture et configuration testables — factory FastAPI, état par instance, configuration typée des chemins serveur et événements d'audit JSON bornés testés ; sous-protocole WebSocket `aetherterm.v1` négocié et rejet d'une connexion sans version testés. Paramètres de l'agent encore à regrouper et à valider au lancement ; séparation supplémentaire du cycle de vie à examiner.
...
- [ ] 6.1 CI bloquante — run [35448700866](https://github.com/OthmaneBlial/AetherTerm/actions/runs/35448700866) vert avec actions figées (Linux Python 3.13, 24 tests dont PTY réel, wheel installé, lint/typecheck, liens docs, assets Web, scans, Chromium, conteneur et binaire agent) ; captures navigateur conservées quand produites ; politique de blocage PR encore requise, en tension avec les pushes directs sur `main` demandés pour cette exécution.
...
- [ ] 6.4 Première release vérifiée — changelog et liste de contrôle de sortie préparés ; aucun tag ni asset publié, portes P0/P1 encore ouvertes.
