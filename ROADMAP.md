...
- [ ] 1.4 Protocole et ressources bornés — schémas, trames, quotas de sessions, sessions opérateur et sockets bornés, démarrage borné à 10 s, envois serveur vers navigateur/agent bornés à 3 s et limiteurs IP distincts ; limite Uvicorn à 64 Kio et formulaire de connexion lu avec plafond de 4 Kio, test de trame excessive isolée. Le run Linux [35447327901](https://github.com/OthmaneBlial/AetherTerm/actions/runs/35447327901) a relayé 1,6 Mio vers un navigateur ne lisant plus : autre navigateur servi sans délai mesurable, +0,2 Mio RSS, 0,10 s CPU et fermeture de session contrôlée. Rafale de reconnexions bornée dans [35447441299](https://github.com/OthmaneBlial/AetherTerm/actions/runs/35447441299). Charge soutenue plus longue et sortie continue encore à mesurer.
...
- [ ] 2.2 Terminal interactif et Unicode exact — xterm.js local, UTF-8 fragmenté, couleurs ANSI, `less` et redimensionnement testés avec Chrome et un PTY macOS ; collage Unicode réel et bouton « Send Ctrl+C » interrompant `cat`, puis commande suivante, vérifiés dans Chrome sur macOS ; sortie Unicode, Ctrl+C, historique Flèche haut et édition/sauvegarde d'un fichier dans Vim validés dans Chromium sur Linux CI [35447709065](https://github.com/OthmaneBlial/AetherTerm/actions/runs/35447709065), avec capture réelle. Autres navigateurs et presse-papiers/toucher physiques restent à valider.
- [ ] 2.3 Reconnexion et états d'erreur — ancien shell terminé et nouvel agent enregistré après redémarrage réel du serveur ; révocation arrête l'agent, backoff réinitialisé après succès ; réauthentification et nouveau shell après redémarrage passés dans le parcours Chromium CI [35445620055](https://github.com/OthmaneBlial/AetherTerm/actions/runs/35445620055) ; coupure réseau du navigateur et délais mesurés à compléter.
...
- [ ] 3.2 Interface accessible et responsive — vrais boutons, focus et sortie lisible par lecteur d'écran ; débordement horizontal absent à 320, 375, 390, 768 et 1280 px dans Chrome local, parcours Chromium CI à 375 px vert et captures réelles bureau/mobile du run [35446357041](https://github.com/OthmaneBlial/AetherTerm/actions/runs/35446357041) ; interaction tactile réelle et revue clavier complète à faire.
...
- [ ] 4.1 Architecture et configuration testables — factory FastAPI, état par instance et événements d'audit JSON bornés testés ; sous-protocole WebSocket `aetherterm.v1` négocié et rejet d'une connexion sans version testés ; configuration typée complète encore requise.
- [ ] 4.2 Suite de tests utile — suite Python couvrant auth, agents, injection, protocole, PTY et reconnexion ; parcours Chromium avec vrai shell, redémarrage, sortie Unicode et interruption passé sur Linux CI ; couverture sensible et répétabilité prolongée encore à revoir.
- [ ] 4.3 Dépendances et maintenance vérifiables — manifeste Python et dépendances directes figées ; scans Python du venv installé et npm exécutés en CI sans avis connu dans le run [35446357041](https://github.com/OthmaneBlial/AetherTerm/actions/runs/35446357041) ; Ruff 0.16.7 passe en CI [35446887099](https://github.com/OthmaneBlial/AetherTerm/actions/runs/35446887099). Résolution transitive, typecheck et scan de secrets encore requis ; Python 3.13 est la seule version déclarée.
- [ ] 5.1 Installation et déploiement reproductibles — guides local, wheel et conteneur écrits ; wheel installé hors checkout et conteneur testé avec un vrai PTY en CI Linux ; démo locale jetable démarrant serveur et agent, avec nettoyage vérifié sur macOS et ajouté à la CI Linux. Exemple de service systemd utilisateur pour boucle locale et contrôle de syntaxe ajoutés ; essai humain Linux propre et cycle de vie du service encore requis.
- [ ] 5.2 Documentation architecture, sécurité et opérations — guides architecture, menaces, sécurité, TLS et opérations rédigés ; signalement privé GitHub activé et vérifié ; revue contradictoire externe et exercice complet de récupération encore requis.
...
- [ ] 6.1 CI bloquante — run [35447441299](https://github.com/OthmaneBlial/AetherTerm/actions/runs/35447441299) vert avec actions figées (Linux Python 3.13, PTY réel, wheel installé, lint, liens docs, assets Web, scans, Chromium, conteneur et binaire agent) ; captures navigateur conservées quand produites ; politique de blocage PR encore requise, en tension avec les pushes directs sur `main` demandés pour cette exécution.
- [ ] 6.2 Artefacts téléchargeables/installables — wheel et sdist conservés avec manifeste SHA-256 dans l'artifact CI [35447341504](https://github.com/OthmaneBlial/AetherTerm/actions/runs/35447341504) ; artifact téléchargé, empreintes vérifiées, wheel installé dans un nouveau venv macOS hors checkout et vrai PTY testé. Serveur conteneur non-root testé sur Linux CI ; binaire agent Linux x86_64 construit, démarré, contrôlé par SHA-256 et utilisé pour un vrai PTY dans [35446843613](https://github.com/OthmaneBlial/AetherTerm/actions/runs/35446843613), mais son téléchargement local n'a pas abouti. Les artifacts CI expirent après sept jours ; installation depuis une release publiée, autres plateformes et second hôte non vérifiés.
- [ ] 6.3 Contributions et présentation GitHub — guide de contribution, politique de sécurité et modèles d'issues/PR ajoutés ; description et neuf topics factuels vérifiés sur la page GitHub, signalement privé activé ; image sociale, liens de release et parcours contributeur externe restent à faire.
...
- **Acceptation :** navigation complète au clavier ; aucune perte de contrôle à 320, 375, 768 et 1280 px ; pas de débordement horizontal du document ni du terminal ; statut et erreurs annoncés sans dépendre seulement de la couleur.
...
- **Fichiers :** nouveaux `assets/demo/source/` non publié si trop lourd/sensible, `docs/DEMO_SCRIPT.md`, référence de commit/tag et commandes de capture ; aucun changement de fonctionnalité pendant la vidéo.
